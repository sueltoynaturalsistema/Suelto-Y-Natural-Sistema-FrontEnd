...
const ProveedorForm = ({ onSave, proveedorInicial, onClose }) => {
  ...
  useEffect(() => {
    if (proveedorInicial) {
      ...
      setMail(proveedorInicial.mail || "");
      setNroTelefono(proveedorInicial.nroTelefono || "");
    } else {
      setNombre("");
      setMail("");
      setNroTelefono("");
  ...
  const handleSubmit = (e) => {
    ...
    onSave({
      id: proveedorInicial?.id || null,
      nombre: nombre.trim(),
      mail: mail.trim(),
      nroTelefono: nroTelefono.trim(),
    });
  };
    ...
    <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center z-[100] p-4">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-md max-h-[95vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div>
            <label className={labelClass}>Nombre / Distribuidora</label>
            <input
              type="text"
